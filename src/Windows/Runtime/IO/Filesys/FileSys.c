...
static inline pntr moduleFn(openStream)(const char* path, word attributes, void* interface, bool create){

	var type = ((stream_Interface*)interface)->type;

	if(
	    type != core.Device.Stream.Type.FILE.type || 
	    type != core.Device.Stream.Type.DIR.type
	){
	...
	rsrcID  result      = -1;
	rsrcID  cachedEntry = Dev.Resource.find(devManager, IO_DevID, generic path);
	...
	if(cachedEntry){
		...
		if(!create){
		    ...
		    if(!entry.exists || entry.type != entryType){
			return nil; // We dont ERR because this comes from a fetch call
		    }
		}

		if(type == core.Device.Stream.Type.FILE.type)
